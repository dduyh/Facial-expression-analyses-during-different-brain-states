...
image = mpimg.imread("D:/duyh/video/20210628 optogenetics_video/CT (Baseline)/sleep_trial5/sleep (337000).jpg")

plt.imshow(image, cmap="gray")

# %%
###example image - cropped
image_crop = image[220:580, 20:580]
...
plt.savefig("D:/duyh/video/20210628 optogenetics_video/CT (Baseline)/sleep_trial5/sleep_crop (337000).png")
# plt.close()     
...
def imagesToHogsCellCrop(imgFolderAndFileType, pixelsPerCell):
    ...
    kwargs = dict(orientations=8, pixels_per_cell=(pixelsPerCell, pixelsPerCell), cells_per_block=(1, 1), transform_sqrt=True, multichannel=True)
    ...
    return Parallel(n_jobs=32)(delayed(hog)(image[220:580, 20:580], **kwargs) for image in coll)
    # return Parallel(n_jobs=32)(delayed(hog)(image, **kwargs) for image in coll)
    
# %%    
#replace the filepath below for own use
fear = imagesToHogsCellCrop("D:/duyh/video/20210628 optogenetics_video/CT (Baseline)/sleep_trial5/*.jpg", 8)
...
corr_fear = fear.iloc[np.r_[0:4000]].T.corr()   

# # %%
writer = pd.ExcelWriter('D:/duyh/video/20210628 optogenetics_video/Exp/sleep_trial12/sleep_trial12_8.xlsx')
...
# %%
Folder = 'D:/duyh/video/20210628 optogenetics_video/Exp/sleep_trial12/'
...
cluster_indices = fcluster(col_linkage, t=3.3, criterion='distance')

dendrogram(col_linkage, no_labels=True, color_threshold=3.3)
plt.axhline(y=3.3, c="k", linestyle='--') 
